...
plugin         = Plugin()
addon          = xbmcaddon.Addon("plugin.video.jadvie.studio")
pluginrootpath = "plugin://plugin.video.jadvie.studio"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.jadvie.studio/(.+?)/.+?\://", item["path"]):
				match = re.search("plugin.video.jadvie.studio(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.jadvie.studio" in item["path"]:
...
def get_playable_url(url):
	if "youtube" in url:
		...
		get_sphim = "https://docs.google.com/spreadsheets/d/13VzQebjGYac5hxe1I-z1pIvMiNB0gSG7oWJlFHWnqsA/export?format=tsv&gid=0"
		try:
			(resp, content) = http.request(
				get_sphim, "GET"
			)
		...
		http.follow_redirects = False
		get_fshare = "https://docs.google.com/spreadsheets/d/12M7whJxHbuni79VVAxIvuddlKKjWnQFyxhYib8y3idU/export?format=tsv&id=12M7whJxHbuni79VVAxIvuddlKKjWnQFyxhYib8y3idU&gid=1368829764"
		try:
			(resp, content) = http.request(
				get_fshare, "GET"
			)
